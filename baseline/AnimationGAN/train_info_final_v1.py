...
parser.add_argument('--name', dest='experiment_name', default='mnist+_gan_sn_3Fc_uniformFlag')
...
transform = torchvision.transforms.Compose(
    [torchvision.transforms.Resize(size=(input_size, input_size), interpolation=Image.BICUBIC),
     torchvision.transforms.ToTensor(),#Img2Tensor [0,255]->[0,1]
...
# #让c_d这个标签递增c_d_num次，每次递增1
# for i in range(sample_num):
# 	sample_d[i] = sample_d[i]*(i%c_d_num+1)

# #----------------------c_c 等差数列 (c_d相等&随机)-------------------- 观察c_c变量，固定其他变量(置0)
...
# temp_c = torch.linspace(-1, 1, c_d_num)*5		#c_d_num个范围在-1->1的等差数列
# sample_c2 = torch.zeros((sample_num, c_c_num))#[200,c_c]

# d_label = np.random.randint(0,c_d_num)
# for i in range(sample_num//c_d_num):		#每c_d个noise,c_d相同,c_c不同
# 	#d_label = random.randint(0,c_d_num-1)
# 	#d_label = np.random.randint(0,c_d_num) #i%c_c_num, 测试第几个d_label
...
# d_label = np.random.randint(0,c_d_num)
# for x in range(sample_num//c_d_num):		#每c_d个noise,c_d相同,c_c不同
# 	#d_label = random.randint(0,c_d_num-1)
# 	#d_label = np.random.randint(0,c_d_num) #x%c_c_num, 测试第几个d_label
# 	sample_c2[x*c_d_num:(x+1)*c_d_num,x%c_c_num] = temp_c

# print(sample_d2[:c_d_num*2])
# print(sample_c2[:c_c_num*2])
...
G = model.generator_mwm_3fc(z_mdim=z_dim_num, img_channel=img_channel, input_size=input_size, len_discrete_code=c_d_num, len_continuous_code=c_c_num)  
D = model.discriminator_mwm_3fc(z_mdim=z_dim_num, img_channel=img_channel, input_size=input_size, len_discrete_code=c_d_num, len_continuous_code=c_c_num, sp_norm=True)
...
if gpu_mode:
	G.cuda()
	D.cuda()
	BCE_loss = nn.BCEWithLogitsLoss().cuda()#nn.BCELoss().cuda()
...
for i in tqdm.trange(epoch):
	...
	for y, c_d_true in tqdm.tqdm(train_loader):
		j = j + 1
		z = torch.rand((batch_size, z_dim_num+1))
		multi_labels = z [:,1:].clone().detach()#one_hot:torch.eye(z_dim_num)[torch.tensor(torch.max(z[:,1:],1)[1]),:]
		multi_labels[multi_labels>0] = 1
		multi_labels[multi_labels != 1] = 0
		if SUPERVISED == True:
			c_d = torch.zeros((batch_size, c_d_num)).scatter_(1, c_d_true.type(torch.LongTensor).unsqueeze(1), 1)
		else:
			c_d = torch.from_numpy(np.random.multinomial(1, c_d_num * [float(1.0 / c_d_num)],size=[batch_size])).type(torch.FloatTensor)*3#投骰子函数,随机化y_disc_即随机的one_hot
		c_d = torch.from_numpy(np.random.exponential(size=(batch_size, c_d_num))).type(torch.FloatTensor)
		c_c = torch.from_numpy(np.random.uniform(-3, 3, size=(batch_size, c_c_num))).type(torch.FloatTensor)
		if gpu_mode:
			y, z, c_d, c_c, multi_labels = y.cuda(), z.cuda(), c_d.cuda(), c_c.cuda(), multi_labels.cuda()
# update D network
		D_optimizer.zero_grad()
		y_f = G(z[:,0].view(batch_size,1),c_d, z[:,1:], c_c)
		D_real, _, _, _ = D(y)
		D_fake, _, _, _ = D(y_f)
...
# # information loss
		...
		G_optimizer.zero_grad()
		y_info = G(z[:,0].view(batch_size,1),c_d, z[:,1:], c_c)
		_,D_disc_info,D_multi_disc_info,D_cont_info = D(y_info)
		...
		multi_disc_loss = BCE_loss(D_multi_disc_info,multi_labels)
		info_loss = disc_loss  + cont_loss*10 + multi_disc_loss
		...
		if ((j + 1) % 100) == 0:
			with open(save_root+'setting.txt', 'a') as f:
				...
				print("D_%s_%s"%(D_real_loss,D_fake_loss),file=f)
				print("G_%s"%(G_loss),file=f)
				print("info_d:%s_md:%s_c:%s"%(disc_loss,multi_disc_loss,cont_loss),file=f)
				print('----',file=f)
				#print("gp: %.8f" %(gradient_penalty.mean()),file=f)

	#----------------------c_c 等差数列 (c_d相等&随机)--------------------
	sample_z1 = torch.zeros(sample_num, 1)
	sample_d2 = torch.zeros(sample_num, c_d_num)#[200,c_d]
	...
	for x in range(sample_num//c_d_num):		#每c_d个noise,c_d相同,c_c不同
		...
		#d_label = np.random.randint(0,c_d_num) #x%c_c_num, 测试第几个d_label
		sample_c2[x*c_d_num:(x+1)*c_d_num,x%c_c_num] = temp_c
	if gpu_mode == True: sample_z1, sample_d2, sample_c2 = sample_z1.cuda(), sample_d2.cuda(), sample_c2.cuda()
            
# save2img
	with torch.no_grad():
		G.eval()
		image_frame_dim = int(np.floor(np.sqrt(sample_num)))
		samples = G(sample_z1, sample_d2, sample_c2, sample_c2)
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir+'/Epoch%d—d.png' % i, nrow=c_d_num)
		samples = G(sample_z1, sample_c2, sample_d2, sample_c2)
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir + '/Epoch%d-dm.png' % i, nrow=c_c_num)
		samples = G(sample_z1, sample_c2, sample_c2, sample_d2)
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir + '/Epoch%d-c.png' % i, nrow=c_c_num)
