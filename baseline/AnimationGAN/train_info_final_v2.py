...
sample_dm = torch.zeros((sample_num, c_dm_num))

#让c_d这个标签递增c_d_num次，每次递增1
for i in range(sample_num):
	sample_d[i] = sample_d[i]*(i%c_d_num+1)

#----------------------c_c 等差数列 (c_d 在每组c_c中相等)--------------------
# 观察c_c变量，固定其他变量(置0)
...
sample_dm_2 = torch.zeros((sample_num, c_dm_num))

#----------------------c_md测试变量--------------------
sample_z3 = torch.rand((1, z_dim_num)).expand(sample_num, z_dim_num) #每个样本的noize相同
sample_d3 = sample_d2
sample_c3 = torch.zeros((sample_num, c_c_num))
sample_dm_3 = np.random.randint(0,c_md_num,sample_num)
sample_dm_3_one_hot = torch.eye(c_md_num)[sample_dm_3,:]
...
G = model.generator_mwm_v2(z_dim=z_dim_num, output_channel=img_channel, input_size=input_size, len_discrete_code=c_d_num, len_continuous_code=c_c_num, len_multi_discrete_code=c_dm_num)  
D = model.discriminator_mwm_v2(input_channel=img_channel, output_dim=1, input_size=input_size, len_discrete_code=c_d_num, len_continuous_code=c_c_num, len_multi_discrete_code=c_dm_num)
...
for i in tqdm.trange(epoch):
	...
	for y, c_d_true in tqdm.tqdm(train_loader):
		...
		c_dm = torch.eye(10)[np.random.randint(0,c_md_num,batch_size),:]
		if gpu_mode:
			y, z, c_d, c_c = y.cuda(), z.cuda(), c_d.cuda(), c_c.cuda(), c_dm.cuda()
# update D network
		D_optimizer.zero_grad()
		y_f = G(z, c_d, c_c, c_dm)
		D_real, _, _ , _ = D(y)
		D_fake, _, _ , _ = D(y_f)
...
# information loss
		...
		G_optimizer.zero_grad()
		y_info = G(z, c_d, c_c, c_dm)
		_,D_disc_info,D_cont_info,D_dm_info = D(y_info)
		...
		dm_loss   = BCE_loss(D_dm_info, c_dm)
		info_loss = disc_loss + cont_loss*20 + dm_loss
...
# save2img
	with torch.no_grad():
		...
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir+'/%d_Epoch—c_d.png' % i, nrow=c_d_num)
		samples = G(sample_z2, sample_d2, sample_c2)
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir + '/%d_Epoch-c_c.png' % i, nrow=c_c_num)
		samples = G(sample_z3.cuda(), sample_d3.cuda(), sample_c3.cuda())
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir + '/%d_Epoch-c_dm.png' % i, nrow=c_dm_num)
